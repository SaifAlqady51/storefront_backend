...
const hash = (password:string):string  => {
   return bcrypt.hashSync(password + BCRYPT_PASSWORD, parseInt(SALT_ROUNDS as string, 10)
  )}
...
describe('User Model Tests', () => {
  ...
  describe('Test User method', () => {
    ...
    it('Test update method', async() => {
      const updatedUser = await userModel.update('2','Eren','Yagear',"titan") as unknown as User;
    ...
    it('Test authenticate success method',async() => {
        ...
        const user = await userModel.authenticate(user_3.first_name,user_3.password) as unknown as User;
