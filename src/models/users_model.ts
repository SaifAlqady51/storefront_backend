...
export class UserModel {
  ...
  async update(id:string,first_name:string,last_name:string,password:string): Promise<User | Message>{
    try{
      const checkID = await checkIfIdExists(id,'users')
      if(!checkID.exists){
        return {message:`user id:${id} does not exist`}
        ...
        const result = await connection.query(sql,[first_name,last_name,hash,id])
        connection.release();
        return result.rows[0];
      }
